...
`optical-train`은 Phase 2 조각으로 collimator 전후와 scanner mirror 반사 후의 `BeamState`, aperture clipping loss, component transmission, mirror reflectivity, rectangle-plane target footprint와 Lambertian virtual-aperture return을 계산한다. 결과는 `results/phase2/<timestamp>_<scenario>_<hash>/` 아래에 `optical_train_report.yaml`과 `optical_train.png`로 저장된다. Baseline의 `receiver.architecture: reciprocal_single_mode_fiber`에서는 `reciprocal_return.path`에 R1 actual hit/closure를, `return_power`에 R2 scalar ledger를, `fiber_coupling`에 R3 mode-overlap을, `detector_boundary`에 R4 passive duplexer와 detector optical input을 기록한다. `power_at_fiber_plane_w`, `power_coupled_into_fiber_w`, `power_at_detector_input_w`와 fiber/target/source 기준 loss를 서로 구분한다. Baseline은 `eta_fiber ≈ 1`, ideal circulator transmission `1`이므로 세 power가 모두 약 `1.80063 nW`이고 source→detector round-trip loss는 약 `67.4457 dB`다. `P_virtual_ap` 약 `2.49999 nW`는 별도 virtual-aperture regression 값이다. Radiometric R3/R4에서 field는 `null`, `coherent_field_status: not_provided`, detector response는 `not_evaluated`다. `scanner.static_command_angle_rad`는 static pose로 적용되며 ideal time sample은 `scanner-path`에서 생성한다. 같은 명령은 짧게 `lidarsim train configs/project.yaml`로도 실행할 수 있다.
...
`workspace`는 현재 Phase 2/R1~R4 결과를 optical assembly workspace용 `ViewportScene`으로 변환한다. Return segment는 `propagation_role: return`으로 송신 ray와 구분하고 R2 plane power를 표시한다. R3 coupling과 R4 detector boundary는 새 광선·부품·beam·field를 만들지 않고 fiber reference-point residual guide metadata에만 표시한다. 계산된 0 W는 `0.0`, 미평가 경로만 `null`이며 return beam radius는 아직 `null`이다. Aperture 또는 plane miss가 있으면 실제 종료점까지만 그린다. `--write-scene`을 주면 strict YAML scene도 함께 저장하며 모든 값은 config와 report에서 나온다.

`dashboard`는 추가 dependency 없이 열 수 있는 self-contained HTML dashboard를 만든다. 같은 실행에서 Phase 2 report YAML, `ViewportScene` YAML, workspace PNG와 optical-train PNG를 함께 저장하고, HTML 안에 virtual aperture, R2 return plane power, R3 fiber coupling과 R4 detector optical boundary를 분리해 표시한다. `--include-scanner-path`를 주면 ideal forward-line scanner path YAML/CSV/PNG도 함께 생성한다. 이 명령은 계속 read-only 결과물이다.
...
`inspect-mesh`는 STL의 unit, bounds, topology, normal, hash와 sidecar metadata를 검사하는 명령이다. Phase 4.1-M1부터 target geometry에는 sidecar의 `asset_id`를 쓰는 안정적인 `geometry.asset_ref`를 권장하며, 기존 `metadata_file`은 project root 기준 상대 경로로 정확히 한 registry sidecar를 가리킬 때만 호환된다. `lidarsim optical-train`은 sidecar unit/world placement를 적용한 CPU/float64 center-ray closest hit를 strict report schema v6의 `stl_intersections`에 기록하고, `workspace`/UI는 `ViewportScene` v2의 STL mesh와 실제 hit marker를 표시한다. R2~R4 power 경로는 rectangle-plane만 지원하며 STL full footprint, target radiometry, BVH, footprint-area occlusion, multi-bounce와 coherent scatterer sampling을 포함하지 않는다.
