...
실제 수신 구조와 다음 물리 구현 순서는 [`docs/specs/RECIPROCAL_FIBER_RETURN.md`](docs/specs/RECIPROCAL_FIBER_RETURN.md)에 정의한다. 기준은 `target → same scanner mirror → same collimator → same single-mode fiber → circulator/coupler → detector`다. Phase 2.4-R1에서 이 경로의 reciprocal center-ray geometry를 구현했지만 반환 파워·fiber mode overlap·detector 입력 파워는 아직 계산하지 않는다. 기존 virtual aperture 계산은 계속 분석·회귀 검증용 중간값으로만 사용한다.
...
2026-07-26 Phase 2-S0, Phase 2-S1, UI-S, Phase 2.4-R1과 Phase 4.1-M1 Gate를 완료했다. 보정 근거, 0 W, nearest-visible energy와 strict schema 계약, actual-hit/no-teleport geometry, scanner pivot, target orientation/sidedness·quadrature, project-wide draft/rollback, reciprocal center-ray와 closure residual에 이어 CPU/float64 STL nearest positive hit를 검증했다. STL sidecar의 unit과 world placement를 적용한 triangle geometry, hit point·geometric normal·distance·triangle ID·front/back face를 strict Phase 2 report schema v3에 기록하고, Plotly/Matplotlib 3D viewport는 strict `ViewportScene` v2의 mesh와 hit marker를 같은 결과에서 표시한다. Rectangle/STL 혼합 scene은 center ray 기준 가장 가까운 target 하나만 visible로 선택한다. 다음 활성 단계는 Phase 2.4-R2 return optical power ledger다.
...
`workspace`는 현재 Phase 2/R1 결과를 optical assembly workspace용 `ViewportScene`으로 변환하고, source/collimator/mirror/target/receiver, local frame, port axis, mirror normal, transmit beam path, target hit, footprint, receiver FOV와 reciprocal return center ray를 하나의 3D PNG로 저장한다. Return segment에는 `propagation_role: return`과 현재 power plane을 아직 평가하지 않았다는 metadata가 붙으며 송신 ray와 다른 선 스타일로 표시된다. Aperture 또는 plane miss가 있으면 실제 종료점까지만 그려 후속 부품으로 순간이동한 것처럼 보이지 않는다. `--write-scene`을 주면 Streamlit/Three.js UI가 소비할 수 있는 strict YAML scene도 함께 저장한다. 이 명령 자체는 read-only viewer이며, 모든 값은 config와 report에서 나온다.
...
`inspect-mesh`는 STL의 unit, bounds, topology, normal, hash와 sidecar metadata를 검사하는 명령이다. Phase 4.1-M1부터 target geometry에는 sidecar의 `asset_id`를 쓰는 안정적인 `geometry.asset_ref`를 권장하며, 기존 `metadata_file`은 project root 기준 상대 경로로 정확히 한 registry sidecar를 가리킬 때만 호환된다. `lidarsim optical-train`은 sidecar unit/world placement를 적용한 CPU/float64 center-ray closest hit를 strict report schema v3의 `stl_intersections`에 기록하고, `workspace`/UI는 `ViewportScene` v2의 STL mesh와 실제 hit marker를 Plotly/Matplotlib에서 표시한다. 이 단계는 STL full footprint, target radiometry, BVH, footprint-area occlusion, multi-bounce와 coherent scatterer sampling을 포함하지 않는다.
