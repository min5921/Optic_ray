...
실제 수신 구조와 다음 물리 구현 순서는 [`docs/specs/RECIPROCAL_FIBER_RETURN.md`](docs/specs/RECIPROCAL_FIBER_RETURN.md)에 정의한다. 기준은 `target → same scanner mirror → same collimator → same single-mode fiber → circulator/coupler → detector`다. Phase 2.4-R1은 reciprocal center-ray geometry를, R2는 nearest-visible rectangle-plane Lambertian target에서 fiber reference plane까지의 scalar return-power ledger를 구현했다. R3는 catalog MFD와 실제 R1 정렬 residual을 사용한 `gaussian_alignment_proxy`로 single-mode overlap 효율과 결합 파워를 계산한다. Duplexer·detector 입력 파워는 R4 범위다. 기존 virtual aperture 계산은 R2/R3와 섞지 않고 분석·회귀 검증용 중간값으로 유지한다.
...
2026-07-27 Phase 2-S0, Phase 2-S1, UI-S, Phase 2.4-R1, Phase 4.1-M1, Phase 2.4-R2와 Phase 2.4-R3 Gate를 완료했다. 보정 근거, 0 W, nearest-visible energy와 strict schema 계약, actual-hit/no-teleport geometry, scanner pivot, target orientation/sidedness·quadrature, project-wide draft/rollback, reciprocal center-ray, CPU/float64 STL nearest positive hit, rectangle-plane return-power ledger와 Gaussian mode 정렬 proxy를 검증했다. R3는 `power_at_fiber_plane_w`에 정규화 overlap 효율을 적용하고 계산된 `0.0`과 미평가 `null`을 분리한다. Strict Phase 2 report schema는 v5이고 `ViewportScene`은 v2다. 다음 활성 단계는 Phase 2.4-R4 duplexer와 detector boundary다.
...
Phase 2의 vertical slice로 source에서 ideal thin-lens collimator를 지나 scanner mirror에서 정지 반사되고, rectangle-plane target footprint와 첫 Lambertian virtual-aperture return까지 계산한다. `lidarsim optical-train`은 free-space propagation, thin-lens ABCD transform, 실제 부품 평면 hit와 projected aperture clipping, off-axis ideal-lens chief ray, static flat-mirror reflection, pivot 기준 static scanner command angle, catalog transmission/reflectivity, target footprint, 분석용 virtual-aperture power와 link budget을 YAML/PNG로 저장한다. 부품 평면이 뒤에 있거나 평행하고 center ray가 clear aperture를 놓치면 component origin으로 광선을 옮기지 않고 0 W `terminated` 경로로 보고한다. Rectangle roll은 `width_axis`, 반사면 앞·뒷면은 material의 `surface_sidedness`가 결정하며 mirror aperture와 target footprint 수치 적분은 base/refined 결과와 수렴 잔차를 함께 보고한다. Phase 2.4-R1은 nearest-visible target hit에서 동일 scanner mirror, collimator receive plane과 source fiber reference plane으로 되돌아가는 center ray를 계산한다. R2는 같은 실제 교차를 Gate로 삼아 Lambertian small-footprint target→projected mirror acceptance, return mirror reflectivity와 reverse collimator transmission을 독립 power ledger로 연결한다. R3는 catalog MFD, actual fiber-plane lateral/angular residual과 configurable mismatch를 사용해 normalized Gaussian overlap과 coupled power를 별도 ledger에 기록한다. Forward 송신 Gaussian aperture clipping fraction은 diffuse return 분포가 아니므로 재사용하지 않는다. Phase 4.1-M1의 STL closest-hit는 계속 geometry-only이며 STL full footprint/radiometry로 확장되지 않는다. Phase 3 reference helper는 static angle sweep과 ideal forward-line scanner path를 지원한다. R3는 Lambertian diffuse return 전체를 deterministic Gaussian으로 둔 optimistic upper-bound/reference이며 calibrated hardware prediction 또는 coherent output이 아니다. Duplexer/detector, calibrated scanner dynamics, STL footprint/occlusion, BRDF, detector noise와 coherent FMCW는 후속 Phase 범위다.
...
`optical-train`은 Phase 2 조각으로 collimator 전후와 scanner mirror 반사 후의 `BeamState`, aperture clipping loss, component transmission, mirror reflectivity, rectangle-plane target footprint와 Lambertian virtual-aperture return을 계산한다. 결과는 `results/phase2/<timestamp>_<scenario>_<hash>/` 아래에 `optical_train_report.yaml`과 `optical_train.png`로 저장된다. Baseline의 `receiver.architecture: reciprocal_single_mode_fiber`에서는 `reciprocal_return.path`에 target→same mirror→collimator receive plane→fiber reference plane의 실제 hit와 closure를, `reciprocal_return.return_power`에 R2 scalar power ledger를, `reciprocal_return.fiber_coupling`에 R3 mode-overlap 결과를 기록한다. `power_at_return_mirror_w`, `power_at_fiber_plane_w`, `fiber_coupling_efficiency`, `power_coupled_into_fiber_w`와 각 plane 사이의 명시적 loss를 서로 구분한다. Baseline은 정렬 proxy `eta_fiber ≈ 1`이므로 fiber-plane과 coupled power가 약 `1.80063 nW`, target→coupled-fiber loss가 약 `67.4457 dB`다. `estimated_received_power_w`, `power_at_virtual_aperture_w`와 CLI의 `P_virtual_ap` 약 `2.49999 nW`는 별도 virtual-aperture regression 값이며 R2/R3 power가 아니다. Radiometric 경로에서는 `coherent_field_status: not_provided`, `coupled_field_amplitude_sqrt_w: null`이며 normalized overlap의 임의 기준 위상을 coherent field로 사용하지 않는다. `detector_status`는 R4 전까지 `not_evaluated`다. `scanner.static_command_angle_rad`는 static pose로 적용되며 ideal time sample은 `scanner-path`에서 생성한다. 같은 명령은 짧게 `lidarsim train configs/project.yaml`로도 실행할 수 있다.
...
`workspace`는 현재 Phase 2/R1/R2/R3 결과를 optical assembly workspace용 `ViewportScene`으로 변환한다. Return segment는 `propagation_role: return`으로 송신 ray와 구분하고, 평가된 R2에서는 target→mirror의 `power_at_return_mirror_w`, mirror→collimator의 `power_after_return_mirror_w`, collimator→fiber의 `power_at_fiber_plane_w`를 표시한다. R3 coupling은 새 광선이나 가짜 field를 만들지 않고 fiber reference-point residual guide의 metadata/hover에만 표시한다. 계산된 0 W는 `0.0`, 미평가 경로만 `null`이며 return beam radius는 아직 계산하지 않아 `null`이다. Aperture 또는 plane miss가 있으면 실제 종료점까지만 그린다. `--write-scene`을 주면 strict YAML scene도 함께 저장하며 모든 값은 config와 report에서 나온다.

`dashboard`는 추가 dependency 없이 열 수 있는 self-contained HTML dashboard를 만든다. 같은 실행에서 Phase 2 report YAML, `ViewportScene` YAML, workspace PNG와 optical-train PNG를 함께 저장하고, HTML 안에 summary, warning, power ledger, target footprint, R2 return plane power와 R3 fiber coupling을 분리해 표시한다. `--include-scanner-path`를 주면 ideal forward-line scanner path YAML/CSV/PNG도 함께 생성한다. 이 명령은 계속 read-only 결과물이다.
...
`lidarsim ui`는 Streamlit browser에서 Plotly 기반 interactive 3D optical bench를 연다. Orbit·zoom, component marker 선택, guide toggle, beam/reflected ray, target hit·footprint, receiver FOV를 한 화면에서 확인하고 선택한 객체를 편집한다. 여러 객체의 미반영 변경은 project-wide draft에 보존되며 변경 객체, field와 YAML diff를 확인하거나 선택/전체 폐기할 수 있다. `ui.autosave_drafts`, `ui.language`, `display_units.power`와 `result_root`가 UI 동작에 연결된다. `변경값 반영 · 시뮬레이션`은 variant YAML·provenance와 완성된 result bundle을 하나의 rollback 단위로 처리하여 validation, simulation 또는 render가 실패하면 이전 성공 파일을 유지한다. 반복 저장은 최초 baseline·직전 parent·현재 variant provenance를 분리하며 ID와 description을 누적하지 않는다. `scan_mirror`의 `MirrorTargetMate`는 추천 pose와 residual을 preview한 뒤 명시적으로 적용한다. Drag/rotate gizmo, undo/redo, port/coaxial snap과 persistent constraint solver는 아직 없다.
...
`inspect-mesh`는 STL의 unit, bounds, topology, normal, hash와 sidecar metadata를 검사하는 명령이다. Phase 4.1-M1부터 target geometry에는 sidecar의 `asset_id`를 쓰는 안정적인 `geometry.asset_ref`를 권장하며, 기존 `metadata_file`은 project root 기준 상대 경로로 정확히 한 registry sidecar를 가리킬 때만 호환된다. `lidarsim optical-train`은 sidecar unit/world placement를 적용한 CPU/float64 center-ray closest hit를 strict report schema v4의 `stl_intersections`에 기록하고, `workspace`/UI는 `ViewportScene` v2의 STL mesh와 실제 hit marker를 표시한다. R2 power ledger는 rectangle-plane만 지원하며 STL full footprint, target radiometry, BVH, footprint-area occlusion, multi-bounce와 coherent scatterer sampling을 포함하지 않는다.
