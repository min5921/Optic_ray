...
이 문서는 목표 물리 구조와 구현 계약을 정리한 것이다. R0, Phase 2-S, UI-S, R1 reciprocal center-ray geometry, Phase 4.1-M1 STL closest-hit와 R2~R4의 rectangle scalar return-power, Gaussian alignment coupling, passive duplexer/detector optical boundary 및 최종 감사를 완료했다. 다음 Gate는 Phase 4.1-M2 STL full-footprint·양방향 occlusion CPU reference다. R2~R4는 nearest-visible Lambertian rectangle의 small-footprint analytical reference와 optimistic Gaussian upper-bound이고 STL radiometry, diffuse spatial-mode decomposition, detector response 또는 coherent field를 계산하지 않는다. 기존 virtual aperture 계산은 regression과 수치 비교를 위해 별도로 유지하며 실제 calibrated fiber-coupled hardware prediction을 주장하지 않는다.
