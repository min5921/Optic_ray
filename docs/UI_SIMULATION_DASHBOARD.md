...
- placement PNG
- report YAML
- ideal forward-line scanner command angle/time
- sample별 target hit coordinate와 received-power trend

아직 실제 장비 계산 결과라고 주장하면 안 되는 항목:

- calibrated/actual scanner trajectory와 bidirectional return stroke
...
- 별도 `lidarsim ui` Streamlit workspace에서 browser button으로 variant validation과 simulation을 실행할 수 있다.
...
- Streamlit은 optional dependency group `ui`로 분리했다.
- `lidarsim ui configs/project.yaml`이 local Streamlit process를 실행한다.
- wavelength, source power, scanner, target와 receiver의 주요 parameter를 form에서 수정한다.
- parameter와 한 component의 catalog/placement edit를 하나의 variant로 함께 저장한다.
- `configs/ui_runs/` nested project가 repository schema/catalog/assets를 올바르게 찾는다.
- validation을 통과한 run만 `results/ui_runs/<scenario>_<hash>/`에 report, 3D workspace, optical-train, optional scanner-path와 standalone dashboard를 생성한다.
...
Phase 3 scanner reference 기능을 UI에 연결하는 단계다. 현재 static pose, angle sweep과 ideal forward-line path가 report/plot 형태로 연결되어 있다.
...
- `scanner-sweep`이 ±amplitude endpoint를 포함한 static angle batch를 YAML/CSV/PNG로 계산한다.
- `scanner-path`가 `static`, `triangle`, `sinusoidal`의 ideal forward line을 frequency와 sample count 기준으로 계산한다.
- `dashboard --include-scanner-path`가 command angle, target coordinate와 received power plot/table을 표시한다.

아직 미구현:

- dynamic lag/jitter
- bidirectional return stroke와 acceleration limit
- `raster`/`custom` waveform
- calibration table/encoder measurement 적용
- browser form의 angle/waveform/sample 편집
...
Phase 3.1 ideal scan samples를 표시하고, 이후 full line/frame 결과로 확장한다.
...
- `scanner-path` YAML/CSV/PNG에 한 줄의 sample time, command angle, target hit와 received power를 저장한다.
- `dashboard --include-scanner-path`가 한 줄 plot과 sample table을 self-contained HTML에 포함한다.

아직 미구현:

- browser에서 waveform과 samples per line을 바꾸는 form
- bidirectional line과 multi-line frame
- footprint trajectory의 interactive 3D animation
- scan-frame heatmap
...
현재 Streamlit workspace:

```text
src/lidarsim/ui/
├─ __init__.py
├─ app.py                 # Streamlit entrypoint와 form
├─ simulation_variant.py  # parameter/component/placement variant + rollback
├─ runner.py              # report·scene·plot·dashboard result bundle
