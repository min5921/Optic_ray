...
2026-07-27 Phase 2-S0, Phase 2-S1, UI-S, Phase 2.4-R1, Phase 4.1-M1과 Phase 2.4-R2 Gate를 완료했다. R2는 configured nearest-visible rectangle footprint와 R1 actual target hit 일치, mirror/collimator/fiber plane 교차를 먼저 검증한 뒤 Lambertian target→projected mirror acceptance, return mirror reflectivity와 reverse collimator transmission을 별도 energy ledger로 계산한다. Strict Phase 2 report schema는 v4이며 `ViewportScene` v2의 actual return segment에 평가된 plane power를 연결한다. STL radiometry는 계속 `not_evaluated`다. 다음 활성 단계는 `Phase 2.4-R3` single-mode fiber coupling이다.
...
Phase 2/R1/M1/R2 vertical slice 상태 (2026-07-27): `src/lidarsim/optics/`에 paraxial `ABCDMatrix`, free-space와 ideal thin-lens q-parameter transform, 실제 ray-plane interaction, projected circular/rectangular aperture clipping, static flat-mirror reflection과 transmitter train propagation을 구현했다. Phase 2.2/2.3은 `rectangle_plane` target footprint, target power와 별도 Lambertian virtual-aperture regression을 계산한다. R1은 nearest-visible configured target hit에서 동일 scanner mirror·collimator receive plane·source fiber reference plane을 실제 교차하고 reverse-oriented thin-lens chief ray와 closure residual을 기록한다. M1은 sidecar unit/world placement가 적용된 STL center-ray closest hit를 geometry-only로 제공한다. R2는 nearest-visible contributing rectangle footprint와 R1 target hit가 `1 nm` tolerance 안에서 일치할 때만 target radiance→projected mirror acceptance를 계산하고, R1 mirror/collimator center-ray aperture `pass`를 1, 나머지를 0으로 매핑한 뒤 catalog mirror reflectivity와 reverse collimator transmission을 순차 적용한다. Forward Gaussian clipping fraction은 diffuse return의 공간 분포가 아니므로 재사용하지 않는다. Baseline의 `power_at_return_mirror_w`와 `power_at_fiber_plane_w`는 약 `1.80063 nW`, target→fiber-plane loss는 약 `67.4457 dB`다. 기존 `P_virtual_ap` 약 `2.49999 nW`는 별도 회귀 중간값이다. Fiber mode overlap, duplexer/detector 전달, STL full footprint/radiometry·BVH·footprint-area occlusion, non-Lambertian BRDF/BSDF, detector noise와 coherent FMCW는 아직 계산하지 않는다. 이 report는 한 개의 static pose만 계산하고 ideal time sample은 Phase 3 `scanner-path`에서 생성한다.
...
세부 구현은 [`specs/RECIPROCAL_FIBER_RETURN.md`](specs/RECIPROCAL_FIBER_RETURN.md)를 따른다. R1 center-ray geometry, Phase 4.1-M1 STL closest-hit와 R2 rectangle-plane return-power ledger를 완료했다. 다음은 R3 fiber coupling과 R4 duplexer/detector boundary를 순서대로 구현한다. R2는 small-footprint scalar analytical reference이며 STL closest-hit만으로 mesh 전체 footprint·BRDF 적분이 완료됐다고 주장하지 않는다.
