...
Phase 2 first vertical slice 상태 (2026-07-08): `src/lidarsim/optics/`에 determinant가 0이 아닌 paraxial `ABCDMatrix`, free-space와 ideal thin-lens q-parameter transform, centered circular aperture clipping과 transmitter train propagation을 추가했다. `lidarsim optical-train`은 active source에서 ideal thin-lens collimator를 거쳐 scanner component origin까지의 element별 `BeamState`, aperture clipping, catalog power transmission, power ledger, ABCD 내부 일관성 check와 PNG를 생성한다. 현재는 ideal centered thin lens와 circular aperture만 지원하며, truncated aperture diffraction, aberration, decenter/tilt tolerance, commercial vendor black-box execution, scanner mirror reflection과 motion은 아직 계산하지 않는다. x/y waist 위치가 분리되는 astigmatic post-lens beam은 현재 `BeamState` contract로 정확히 표현할 수 없으므로 silent approximation 대신 명시적으로 거부한다.
