...
10. Return aperture를 키우면 expected regime에서 accepted power가 증가하고 fiber mode mismatch가 커지면 coupled power가 감소한다.
...
현재 virtual aperture result는 위 경로를 구현하기 위한 회귀 기준으로 유지한다. 왕복 center-ray geometry, return power ledger와 fiber mode coupling이 analytical result와 일치한 뒤 sequential prescription, vendor file import, STEP assembly, complex BSDF, coherent FMCW와 GPU 순서로 확장한다.
