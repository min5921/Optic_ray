...
UI early slice 상태 (2026-07-10): `ViewportScene` 기반 Matplotlib 3D viewer와 self-contained read-only dashboard에 이어, optional Streamlit `lidarsim ui` browser workspace를 구현했다. 사용자는 주요 source/scanner/target/receiver parameter, compatible component reference와 absolute/port numeric placement를 편집할 수 있다. 변경은 `configs/ui_runs/` variant YAML로 serialize하고 schema/unit/physical/placement validation을 통과한 뒤 `results/ui_runs/` report·PNG·HTML bundle을 생성하므로 CLI에서 같은 project를 재현할 수 있다. 아직 interactive 3D picking, snapping, mate/constraint, undo/redo, drag/rotate gizmo, multi-run comparison과 tolerance analysis는 구현하지 않았다.
