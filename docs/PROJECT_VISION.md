...
이 프로젝트의 목적은 사용자가 정의하거나 상용 catalog에서 선택한 광원, collimator, lens, aperture, mirror와 scanner를 실제 공간에 배치하고, 그 광학계를 통과한 빔이 시간에 따라 어디로 진행하는지 계산하는 것이다. 이후 빔이 물체에 만드는 footprint와 irradiance를 구하고, 물체의 형상과 재질에 따라 반환된 광이 동일 scanner와 collimator를 역으로 통과해 single-mode fiber에 얼마만큼 결합되는지, 또는 coherent field가 어떻게 합산되는지 예측한다.
...
Material BSDF / BRDF / roughness response
    ↓
Reverse scanner/collimator optical train
    ↓
Single-mode fiber coupling and duplexer
    ↓
Detector or coherent mixer
    ↓
...
기준 구성은 송신에 사용한 scanner mirror와 collimator를 수신에도 다시 사용하는 reciprocal monostatic single-mode fiber architecture다. 상세 물리 contract와 단계별 구현 순서는 [`specs/RECIPROCAL_FIBER_RETURN.md`](specs/RECIPROCAL_FIBER_RETURN.md)를 따른다.

```text
Tx: fiber/source → shared collimator → shared scanner mirror → target
Rx: target → same scanner mirror → same collimator → same fiber receive mode
    → circulator/coupler → detector 또는 coherent mixer
```

현재 `virtual_monostatic/virtual_aperture` 계산은 실제 왕복 파워 모델과 분리된 analytical regression intermediate다. Phase 2.4-R1은 target→same scanner mirror→same collimator→same fiber reference plane의 reciprocal center-ray geometry를 구현했고, R2는 nearest-visible rectangle-plane Lambertian target의 small-footprint scalar power를 실제 R1 aperture/intersection Gate를 따라 fiber reference plane까지 전달한다. R3는 catalog MFD와 actual/configured 정렬 residual을 사용하는 `gaussian_alignment_proxy`로 fiber-coupled power를 계산하고, R4는 configured passive duplexer transmission을 적용해 detector optical input boundary까지 전달한다. R4는 detector의 별도 공간 배치나 response model이 아니며 responsivity, photocurrent, noise, saturation과 coherent field를 계산하지 않는다. R2~R4는 analytical/uncalibrated reference이고 forward Gaussian clipping fraction을 diffuse return에 재사용하지 않는다. `estimated_received_power_w`와 `power_at_virtual_aperture_w`는 여전히 virtual aperture plane의 같은 회귀 기준값으로 R2 power, fiber 결합 power 또는 detector input power가 아니다.

Receiver optical 입력:

- reciprocal return path와 target/scanner/collimator/fiber element reference
- scanner mirror와 collimator의 bidirectional port/reference plane
- return aperture와 optical transmission/reflectivity
- fiber MFD, receive mode, lateral/angular/focus mismatch
- circulator/coupler architecture와 insertion loss/isolation
- optional independent receiver position/direction와 placement transform
- optional aperture shape/diameter, FOV와 field stop
- optional separate receive lens prescription
- optical efficiency와 coating
- focal length와 detector size
- fiber core/NA/LO mode와 polarization 조건
...
```text
target return
→ power/field accepted by return scanner mirror
→ power/field after return collimator
→ single-mode fiber overlap and coupled power/field
→ power/field after circulator or coupler
→ power at detector or coherent mixer
...
- return mirror/collimator aperture, visibility와 reciprocity residual
- fiber coupling efficiency와 coupled optical power
- plane이 명시된 intermediate optical power와 link loss
...
- virtual-aperture, return-mirror, return-collimator와 fiber-coupled power 구분
- fiber coupling efficiency와 lateral/angular/focus mismatch
- detector power vs time/pixel/range
...
### 16.1 현재 활성 구현 순서 — 2026-07-15 검수 반영

기존 Phase 번호는 장기 기능 영역을 유지한다. 실제 착수 순서는 기능 간 선행조건을 반영해 다음과 같이 고정한다.

```text
Phase 2-S0 신뢰도·계약 안정화
→ Phase 2-S1 실제 ray-plane/port 배치 geometry
→ UI-S 편집·저장 안정화
...
→ calibrated scanner dynamics / advanced material / coherent FMCW
```
...
2026-07-27 Phase 2-S0, Phase 2-S1, UI-S, Phase 2.4-R1, Phase 4.1-M1과 Phase 2.4-R2~R4 Gate 및 최종 감사를 완료했다. R2는 configured nearest-visible rectangle footprint와 R1 actual target hit 일치, mirror/collimator/fiber plane 교차를 먼저 검증한 뒤 Lambertian target→projected mirror acceptance, return mirror reflectivity와 reverse collimator transmission을 별도 energy ledger로 계산한다. R3는 catalog MFD, R1 actual lateral/angular residual과 configured mismatch를 사용하는 `gaussian_alignment_proxy`를 별도 coupling ledger로 연결한다. R4는 R3 coupled power에 passive duplexer transmission을 적용하고 detector optical input boundary에서 끝난다. Strict Phase 2 report schema는 v6이며 `ViewportScene` v2는 R2 actual return segment, R3 coupling과 R4 boundary metadata를 분리한다. Detector response와 coherent field는 `not_evaluated`/`not_provided`이고 STL radiometry/coupling도 계속 `not_evaluated`다. 감사 결과는 [`reports/PHASE_2_4_R4_COMPLETION_AUDIT_2026-07-27.md`](reports/PHASE_2_4_R4_COMPLETION_AUDIT_2026-07-27.md)에 기록했으며 다음 활성 Gate는 Phase 4.1-M2 STL full-footprint·양방향 occlusion CPU reference다.
...
Phase 2/R1/M1/R2~R4 vertical slice 상태 (2026-07-27): `src/lidarsim/optics/`에 paraxial `ABCDMatrix`, free-space와 ideal thin-lens q-parameter transform, 실제 ray-plane interaction, projected circular/rectangular aperture clipping, static flat-mirror reflection과 transmitter train propagation을 구현했다. Phase 2.2/2.3은 `rectangle_plane` target footprint, target power와 별도 Lambertian virtual-aperture regression을 계산한다. R1은 nearest-visible configured target hit에서 동일 scanner mirror·collimator receive plane·source fiber reference plane을 실제 교차하고 reverse-oriented thin-lens chief ray와 closure residual을 기록한다. M1은 sidecar unit/world placement가 적용된 STL center-ray closest hit를 geometry-only로 제공한다. R2는 nearest-visible contributing rectangle footprint와 R1 target hit가 `1 nm` tolerance 안에서 일치할 때만 target radiance→projected mirror acceptance를 계산하고, R1 mirror/collimator center-ray aperture `pass`를 1, 나머지를 0으로 매핑한 뒤 catalog mirror reflectivity와 reverse collimator transmission을 순차 적용한다. R3는 R2 `power_at_fiber_plane_w`, catalog MFD와 actual/configured 정렬 mismatch를 정규화 Gaussian overlap에 연결한다. R4는 R3 `power_coupled_into_fiber_w`에 configured passive duplexer return transmission을 적용하고 detector input optical boundary의 power와 fiber/target/source 기준 loss를 별도 ledger로 기록한다. Forward Gaussian clipping fraction은 diffuse return의 공간 분포가 아니므로 재사용하지 않는다. Baseline의 `power_at_return_mirror_w`, `power_at_fiber_plane_w`, `power_coupled_into_fiber_w`와 `power_at_detector_input_w`는 약 `1.80063 nW`, `eta_fiber ≈ 1`, source→detector round-trip loss는 약 `67.4457 dB`다. 기존 `P_virtual_ap` 약 `2.49999 nW`는 별도 회귀 중간값이다. R3/R4는 diffuse-return analytical upper-bound/reference이고 coherent field를 제공하지 않는다. Detector responsivity·photocurrent·noise·saturation, STL full footprint/radiometry·coupling·BVH·footprint-area occlusion, non-Lambertian BRDF/BSDF와 coherent FMCW는 아직 계산하지 않는다. 이 report는 한 개의 static pose만 계산하고 ideal time sample은 Phase 3 `scanner-path`에서 생성한다.

#### Phase 2-S — R1 착수 전 안정화 Gate

- calibration evidence 없이 `model_purpose`만으로 `calibrated` 또는 overall `pass`를 선언하지 않는다.
- collimator·mirror placement에서는 축상 거리와 경고만 계산한 뒤 component origin으로 beam을 옮기지 않고 실제 ray-plane/port hit를 계산한다.
- off-axis, tilt, aperture miss는 power loss 또는 terminated path로 명시한다.
- 여러 target의 독립 후보 power를 scene energy total로 중복 합산하지 않는다.
- schema와 runtime의 zero transmission·zero-power 범위를 일치시킨다.
- scanner axis, target normal, receiver direction의 finite/non-zero 의미와 scanner pivot을 검증한다.
- UI의 project-wide draft, atomic variant run, stable provenance와 footprint world orientation을 보강한다.

Phase 2-S 완료 조건: calibration gate, zero-power, nearest-visible target energy, off-axis lens/mirror miss·clipping, scanner pivot, target orientation/sidedness와 quadrature convergence analytical regression이 모두 통과한다. UI draft/rollback은 바로 다음 `UI-S` Gate에서 별도로 닫는다.

2026-07-23 Phase 2-S1 완료: 공통 ray-plane 교차, no-teleport termination, off-axis ideal-lens chief ray와 scanner pivot을 구현했다. `geometry.width_axis`와 right-handed target frame, material `one_sided/two_sided` backface·radiometric-normal 정책을 geometry와 receiver return에 공통 적용했다. Mirror/footprint 적분은 base/refined 수렴 residual을 보고하고 tolerance 초과를 warning으로 올린다. 다음 활성 단계는 `UI-S`다.
...
### Phase 5 — Material과 Reciprocal Fiber Receiver

- diffuse/specular/retro material
- BRDF link budget
- return scanner/collimator aperture와 reverse optical train
- single-mode fiber mode overlap과 duplexer
- optional independent receiver aperture/FOV/receive optics
- material/path contribution report

세부 구현은 [`specs/RECIPROCAL_FIBER_RETURN.md`](specs/RECIPROCAL_FIBER_RETURN.md)를 따른다. R1 center-ray geometry, Phase 4.1-M1 STL closest-hit와 R2~R4의 rectangle-plane return-power ledger, Gaussian alignment coupling proxy, passive duplexer/detector optical input boundary를 완료했다. R2~R4는 small-footprint scalar analytical reference와 optimistic mode-alignment upper-bound이며 STL closest-hit만으로 mesh 전체 footprint·BRDF·spatial-mode 적분이 완료됐다고 주장하지 않는다. R4는 detector response가 아닌 optical boundary이므로 calibrated responsivity, photocurrent, noise, saturation 또는 coherent mixer 입력을 제공하지 않는다.
...
UI-S 완료 상태 (2026-07-26): 선택을 바꿔도 여러 객체 편집을 보존하는 `ProjectDraft`, 변경 객체/field/YAML diff, 선택·전체 discard를 구현했다. Variant·project·provenance와 result directory는 validation→simulation→render 실패 시 이전 성공 상태로 rollback한다. 최초 baseline, 직전 parent와 현재 variant identity를 sidecar에 분리하고 반복 저장 ID·description 누적을 막았다. Projected Gaussian eigensystem의 major/minor local·world axis를 report·strict schema·Plotly·Matplotlib이 공유하며, `result_root`, language, autosave와 power display unit을 실제 UI 동작에 연결했다. R1 return path는 `propagation_role: return`과 closure residual guide로 actual hit를 표시하고, 2026-07-27 R2부터 평가된 segment에는 plane별 scalar power를 연결한다. R3 coupling과 R4 detector optical boundary는 fiber reference-point metadata와 별도 CLI/Streamlit/dashboard metric으로 표시하며 새 ray·component·beam·field를 만들지 않는다. 계산된 0 W는 `0.0`, 미평가 power와 return beam radius만 `null`이다.
...
- 대규모/복합 STL·STEP scene, BVH, occlusion와 multiple optical paths
...
9. Calibration evidence가 없는 config는 `calibrated` confidence 또는 hardware readiness를 만들지 않는다.
10. Off-axis 또는 tilted lens·mirror에서 beam이 component origin으로 순간이동하지 않고 실제 hit, clipping 또는 miss가 보고된다.
11. 여러 target 후보가 있어도 nearest-visible hit 정책과 scene energy ledger가 송신 power를 중복 사용하지 않는다.
12. Zero transmission 또는 완전 aperture rejection에서 path power가 0이 되고 report/schema가 유효하다.
13. Scanner axis·target normal·receiver direction의 zero vector가 load 단계에서 거부되고 scanner pivot 회전이 분석값과 일치한다.
14. UI에서 여러 객체를 편집한 draft가 보존되고 실패한 simulation은 기존 variant를 변경하지 않는다.

### Phase 3-6: Scanner, Target와 Radiometry

15. Line beam을 mirror로 scan하고 line 위치/orientation이 분석값과 일치한다.
16. Reciprocal exact-retrace case가 같은 mirror·collimator·fiber port로 돌아오고 closure residual tolerance를 만족한다.
17. 평면 2-triangle STL의 nearest hit point·normal·distance가 `rectangle_plane` 분석값과 일치한다.
18. Return aperture를 키우면 expected regime에서 accepted power가 증가하고 fiber mode mismatch가 커지면 coupled power가 감소한다.
19. Diffuse, mirror, black material와 retroreflector return이 구분된다.
20. Distance 증가에 따른 power 감소가 선택한 radiometric model과 일치한다.
21. Wavelength/component swap이 effective config, compatibility warning와 comparison report에 추적된다.
22. Mesh/footprint/time sampling을 refine했을 때 selected metric이 수렴한다.
23. Catalog nominal/tolerance/measurement provenance가 result까지 추적된다.

### Phase 7-8: Advanced Physics

24. 같은 seed에서 roughness/speckle/tolerance result가 재현된다.
25. 10 m target FMCW range가 한 range bin 안에 있다.
26. Detector saturation/noise threshold가 analytical case와 일치한다.
27. Area-beam power integral과 Fresnel propagation이 선택한 reference case와 일치한다.
28. CPU/GPU와 precision 변화가 declared metric tolerance를 만족한다.
...
- [x] return mirror·collimator aperture power ledger
- [x] CPU STL target closest-hit와 2-triangle plane parity 검증
- [x] single-mode fiber Gaussian alignment proxy와 coupled power
...
- [ ] 실제 shared collimator/scanner/fiber와 duplexer part number 및 손실
- [ ] 동일 송수신 fiber 또는 별도 receive fiber 여부
- [ ] fiber MFD/NA, 정렬 tolerance, detector와 target material data
...
→ flat diffuse target
→ trust/energy/zero-power and actual ray-plane placement gates
→ reverse intersection with the same scanner mirror
→ reverse traversal through the same collimator
→ CPU STL target closest-hit parity slice
→ return mirror/collimator optical power ledger
→ single-mode fiber overlap and coupled power
→ ideal configurable circulator/coupler boundary
→ 3D component layout
→ beam envelope, round-trip path, footprint, fiber-coupled power and link budget
```

현재 virtual aperture result는 R2와 별도인 회귀 기준으로 유지한다. [`specs/IMPLEMENTATION_AUDIT_2026-07-15.md`](specs/IMPLEMENTATION_AUDIT_2026-07-15.md)의 안정화 Gate, 왕복 center-ray geometry, CPU STL closest-hit parity와 rectangle-plane return power ledger를 완료했다. 다음 fiber mode coupling과 duplexer/detector boundary가 analytical result와 일치하면 sequential prescription, vendor file import, STEP assembly, complex BSDF, coherent FMCW와 GPU 순서로 확장한다.
...
- [`specs/RECIPROCAL_FIBER_RETURN.md`](specs/RECIPROCAL_FIBER_RETURN.md)
- [`specs/IMPLEMENTATION_AUDIT_2026-07-15.md`](specs/IMPLEMENTATION_AUDIT_2026-07-15.md)
