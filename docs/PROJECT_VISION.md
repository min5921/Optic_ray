...
2026-07-27 Phase 2-S0, Phase 2-S1, UI-S, Phase 2.4-R1, Phase 4.1-M1, Phase 2.4-R2와 Phase 2.4-R3 Gate를 완료했다. R2는 configured nearest-visible rectangle footprint와 R1 actual target hit 일치, mirror/collimator/fiber plane 교차를 먼저 검증한 뒤 Lambertian target→projected mirror acceptance, return mirror reflectivity와 reverse collimator transmission을 별도 energy ledger로 계산한다. R3는 catalog MFD, R1 actual lateral/angular residual과 configured mismatch를 사용하는 `gaussian_alignment_proxy`를 별도 coupling ledger로 연결한다. Strict Phase 2 report schema는 v5이며 `ViewportScene` v2는 actual return segment의 R2 plane power와 fiber reference-point R3 metadata를 분리한다. STL radiometry/coupling은 계속 `not_evaluated`다. 다음 활성 단계는 `Phase 2.4-R4` duplexer와 detector boundary다.
...
Phase 2/R1/M1/R2/R3 vertical slice 상태 (2026-07-27): `src/lidarsim/optics/`에 paraxial `ABCDMatrix`, free-space와 ideal thin-lens q-parameter transform, 실제 ray-plane interaction, projected circular/rectangular aperture clipping, static flat-mirror reflection과 transmitter train propagation을 구현했다. Phase 2.2/2.3은 `rectangle_plane` target footprint, target power와 별도 Lambertian virtual-aperture regression을 계산한다. R1은 nearest-visible configured target hit에서 동일 scanner mirror·collimator receive plane·source fiber reference plane을 실제 교차하고 reverse-oriented thin-lens chief ray와 closure residual을 기록한다. M1은 sidecar unit/world placement가 적용된 STL center-ray closest hit를 geometry-only로 제공한다. R2는 nearest-visible contributing rectangle footprint와 R1 target hit가 `1 nm` tolerance 안에서 일치할 때만 target radiance→projected mirror acceptance를 계산하고, R1 mirror/collimator center-ray aperture `pass`를 1, 나머지를 0으로 매핑한 뒤 catalog mirror reflectivity와 reverse collimator transmission을 순차 적용한다. R3는 R2 `power_at_fiber_plane_w`, catalog MFD와 actual/configured 정렬 mismatch를 정규화 Gaussian overlap에 연결한다. Forward Gaussian clipping fraction은 diffuse return의 공간 분포가 아니므로 재사용하지 않는다. Baseline의 `power_at_return_mirror_w`, `power_at_fiber_plane_w`와 `power_coupled_into_fiber_w`는 약 `1.80063 nW`, `eta_fiber ≈ 1`, target→coupled-fiber loss는 약 `67.4457 dB`다. 기존 `P_virtual_ap` 약 `2.49999 nW`는 별도 회귀 중간값이다. R3는 diffuse-return Gaussian upper-bound/reference이고 coherent field를 제공하지 않는다. Duplexer/detector 전달, STL full footprint/radiometry·coupling·BVH·footprint-area occlusion, non-Lambertian BRDF/BSDF, detector noise와 coherent FMCW는 아직 계산하지 않는다. 이 report는 한 개의 static pose만 계산하고 ideal time sample은 Phase 3 `scanner-path`에서 생성한다.
...
세부 구현은 [`specs/RECIPROCAL_FIBER_RETURN.md`](specs/RECIPROCAL_FIBER_RETURN.md)를 따른다. R1 center-ray geometry, Phase 4.1-M1 STL closest-hit, R2 rectangle-plane return-power ledger와 R3 Gaussian alignment coupling proxy를 완료했다. 다음은 R4 duplexer/detector boundary다. R2/R3는 small-footprint scalar analytical reference와 optimistic mode-alignment upper-bound이며 STL closest-hit만으로 mesh 전체 footprint·BRDF·spatial-mode 적분이 완료됐다고 주장하지 않는다.
...
UI-S 완료 상태 (2026-07-26): 선택을 바꿔도 여러 객체 편집을 보존하는 `ProjectDraft`, 변경 객체/field/YAML diff, 선택·전체 discard를 구현했다. Variant·project·provenance와 result directory는 validation→simulation→render 실패 시 이전 성공 상태로 rollback한다. 최초 baseline, 직전 parent와 현재 variant identity를 sidecar에 분리하고 반복 저장 ID/description을 안정화했다. Projected Gaussian eigensystem의 major/minor local·world axis를 report·strict schema·Plotly·Matplotlib이 공유하며, `result_root`, language, autosave와 power display unit을 실제 UI 동작에 연결했다. R1 return path는 `propagation_role: return`과 closure residual guide로 actual hit를 표시하고, 2026-07-27 R2부터 평가된 segment에는 plane별 scalar power를 연결한다. R3 coupling은 fiber reference-point metadata와 별도 CLI/Streamlit/dashboard metric으로 표시하며 새 ray·beam·field를 만들지 않는다. 계산된 0 W는 `0.0`, 미평가 power와 return beam radius만 `null`이다.
