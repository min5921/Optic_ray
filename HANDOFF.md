...
- R3는 Lambertian diffuse-return optimistic upper-bound/reference이며 calibrated hardware가 아니다. `coherent_field_status: not_provided`, `coupled_field_amplitude_sqrt_w: null`, `field_usable_for_coherent_propagation: false`를 유지한다.
- R3 core/UI/CLI focused suite `tests/test_fiber_coupling.py tests/test_reciprocal_project.py tests/test_return_power_project.py tests/test_optical_train.py tests/test_stl_project.py tests/test_ui_app.py tests/test_ui_workspace.py tests/test_cli.py`: normal/strict 모두 통과, 각각 `112 passed`.
- `python -m lidarsim.cli validate configs/project.yaml`, `optical-train`, `workspace --write-scene`와 `dashboard` smoke: 모두 통과. Viewport YAML의 fiber residual metadata와 HTML의 R3 별도 section에서 model/efficiency/power 및 `coherent_field_status: not_provided`를 확인했다.
- R3 checkpoint 전체 suite `python -m pytest -q --ignore=tests/test_detector_boundary.py`: 통과, `300 passed`.
- R3 checkpoint 전체 strict warning suite `python -W error::DeprecationWarning -W error::UserWarning -m pytest -q --ignore=tests/test_detector_boundary.py`: 통과, `300 passed`.
...
- R2 checkpoint 자체는 STL radiometry와 fiber coupling 이후 단계를 포함하지 않았다. 후속 R3 fiber coupling은 현재 완료했고 R4 duplexer/detector와 coherent FMCW는 아직이다.
